...
beforeEach(() => {
  ...
  writePrimitive(TEST_DIR, 'rules', 'typescript.md',
  ...
  writePrimitive(TEST_DIR, 'rules', 'deprecated-skill.md',
    `---\nid: deprecated-skill\ntags: [old]\nstatus: deprecated\nauthor: human\ndescription: An old skill\n---\n# Skill: Old\n\nThis skill is deprecated.`);

  writePrimitive(TEST_DIR, 'rules', 'be-concise.md',
...
describe('searchPrimitives', () => {
  ...
  it('should filter by type directory', () => {
    ...
    // After primitive collapse, typescript and be-concise moved from instincts → rules.
    // All 4 active/draft primitives now live in rules.
    expect(results).toHaveLength(4);
  ...
  it('should combine multiple filters', () => {
    ...
    // After primitive collapse, be-concise (author: agent) moved from instincts → rules.
    // Now both testing and be-concise are in rules with author: agent.
    expect(results).toHaveLength(2);
    const ids = results.map((r) => r.doc.id).sort();
    expect(ids).toEqual(['be-concise', 'testing']);
  });
  ...
  it('should include directory info in results', () => {
    const results = searchPrimitives(TEST_DIR, 'typescript');
    expect(results).toHaveLength(1);
    expect(results[0].directory).toBe('rules');
  });
