...
- [x] **12.13 COMPLETE** — Current shipped version: `@agntk/agent-harness@0.1.2` (run `24137570409`, commit `1547986`). v0.1.0 was YANKED due to license issue (see post-mortem). v0.1.1 shipped the license fix but had a hardcoded CLI version string bug (`harness --version` printed `0.1.0`). v0.1.2 fixes the version bug by reading `package.json.version` at runtime via `createRequire`. See post-ship discoveries below for the full sequence.
...
- **12.13 (post-ship 1)**: Discovered at push time — `agent-harness` was taken on npm by an unrelated placeholder package (`quuu/agent-harness@0.0.1`). Renamed to `@randywilson/agent-harness` in `package.json`. Bin name still `harness`. Scoped packages require `npm publish --access public` which the release workflow already specifies.
...
- **POST-SHIP (v0.1.1 version bug)**: Within minutes of v0.1.1 going live, post-ship clean-install verification caught that `harness --version` printed `0.1.0` instead of `0.1.1`. Root cause: `.version('0.1.0')` was hardcoded in `src/cli/index.ts` as a string literal. Bumping `package.json` did nothing because the CLI had its own copy. Fixed in `1547986` by reading `package.json.version` at runtime via `createRequire(import.meta.url)('../../package.json')`. Works because npm always includes `package.json` in published tarballs (special-cased), and its path relative to `dist/cli/index.js` is deterministic. Future version bumps flow through automatically with zero source edits. Shipped as v0.1.2 since npm won't let you republish the same version within 24 hours of an unpublish, so a forward bump was the cleanest recovery.
...
- [x] **12.14 COMPLETE** — All three actionable levels shipped. Level 1 (provenance) → v0.1.3 commit `0525d52`. Level 2 (license detection) → v0.1.4 commit `10fd002`. Level 3 (policy enforcement) → v0.1.5 commit `b982cb9`. Level 4 (doctor audit) deferred to v0.2.0 per the original spec. The v0.1.0 yank scenario is now mechanically prevented: `harness install <anthropics raw URL>` is blocked with a clear error message naming the per-file LICENSE.txt and offering `--force-license` as an explicit override.

## Discoveries from 12.14 implementation

- **L1 install_by version bug**: tsup bundles `universal-installer.ts` flat into `dist/<bundle>.js`, not `dist/runtime/`. The original `createRequire('../../package.json')` resolved to one level above the repo root and returned `'unknown'`. Fixed by walking multiple candidate paths (`../`, `../../`, `../../../`) and validating `pkg.name === '@agntk/agent-harness'` to find the right one.
- **L1 gray-matter content cache**: gray-matter caches parses by input string content. Tests with identical inputs got the SAME mutated `data` object back from cache, causing test 1's mutations to leak into test 2. Fixed by making each test's input unique with `Math.random().toString(36).slice(2, 10)` in the id field. **Worth documenting for any future tests that exercise functions that mutate parsed frontmatter.**
- **L2 copyright regex `\b` after `©`**: My initial regex `^(©|copyright)\b` failed on `© 2025...` because `\b` requires a word-char boundary, and `©` is a symbol (non-word char) followed by space (also non-word) → no boundary. Fixed with two separate alternations: `^©\s*\d{4}` OR `^copyright\b.*\d{4}`.
- **L2 GitHub License API has the body inline**: the `/repos/.../license` endpoint returns the LICENSE text as base64 in `content` (with `encoding: 'base64'`). No second fetch needed to extract the copyright line — decode the body in the same call.
